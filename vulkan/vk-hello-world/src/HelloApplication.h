...
private:
    ...
    void createCommandBuffers() {
        ...
        for (size_t i = 0; i < commandBuffers.size(); i++) {
            ...
            vkCmdBeginRenderPass(commandBuffers[i], &renderPassInfo, VK_SUBPASS_CONTENTS_INLINE);
                ...
                if (mesh != NULL) mesh->bind(commandBuffers[i]);
    ...
    void updateUniformBuffer(uint32_t currentImage) {
        if (mesh != NULL) {
            mesh->updateUniformBuffer(device, swapChainExtent, currentImage);
        }
    }
    ...
    QueueFamilyIndices findQueueFamilies(VkPhysicalDevice device) {
        ...
        std::cout << "presentFamily: " << indices.presentFamily.value() << std::endl;
    ...
    void createDescriptorSets() {
        ...
        for (size_t i = 0; i < swapChainImages.size(); i++) {
            ...
            bufferInfo.buffer = mesh->uniformBuffers[i];//uniformBuffers[i];
            bufferInfo.offset = 0;
